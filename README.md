...
Instagram-ссылки их kkclip-версией — превью в Telegram подгружается, видео
проигрывается inline без открытия Instagram.

Конвертация — простая замена хоста: `instagram.com/<path>` →
`<TARGET_HOST>/<path>`. Целевой хост настраивается через `.env`; если kkclip
перестанет работать, достаточно поменять `TARGET_HOST` и перезапустить сервис.
